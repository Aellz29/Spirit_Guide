...
if (!in_array($category, $allowed)) {
  die("<div class='h-screen flex items-center justify-center'><h2 class='text-3xl font-black text-gray-300 uppercase tracking-widest'>Kategori Tidak Ditemukan</h2></div>");
}
        ...
        <div id="productGrid" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 fade-in" style="animation-delay: 0.2s;">
            ...
            <div class="product-card group bg-white rounded-3xl overflow-hidden relative border border-gray-100 flex flex-col h-full">
                <div class="absolute top-4 left-4 z-20 flex flex-col gap-2 items-start">
                    <?php if($p['is_flash_sale']): ?><span class="pulse-badge bg-red-600 text-white text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg flex items-center gap-1"><i class="fa fa-bolt"></i> Sale</span><?php endif; ?>
                    <?php if($showMemberPrice): ?><span class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg flex items-center gap-1"><i class="fa fa-crown"></i> Member</span><?php endif; ?>
                    <?php if($diskonPersen > 0): ?><span class="bg-black text-white text-[10px] font-bold px-2 py-1 rounded-lg shadow-md">-<?= $diskonPersen ?>%</span><?php endif; ?>
                </div>

                <div class="cursor-pointer relative overflow-hidden bg-gray-50 aspect-[4/5]" onclick="openModal(
                    '<?= htmlspecialchars($p['image']) ?>', '<?= htmlspecialchars(addslashes($p['title'])) ?>', '<?= $priceDisplay ?>', '<?= $originalDisplay ?>', '<?= htmlspecialchars(addslashes($p['description'])) ?>', <?= (int)$p['stock'] ?>, <?= $p['id'] ?>, <?= $finalPrice ?>, <?= $p['is_flash_sale'] ? 1 : 0 ?>, <?= $showMemberPrice ? 1 : 0 ?>
                )">
                    ...
                    <div class="pt-4 border-t border-gray-50 mt-auto">
                        <div class="flex justify-between items-end mb-4">
                            <div class="flex flex-col">
                                <?php if($hargaCoret > 0): ?><span class="text-[11px] text-gray-400 line-through font-medium mb-0.5">Rp <?= number_format($hargaCoret, 0, ',', '.') ?></span><?php endif; ?>
                            ...
                            <?php if($p['stock'] < 5 && $p['stock'] > 0): ?><span class="text-[9px] font-bold text-red-500 bg-red-50 px-2 py-1 rounded">Sisa <?= $p['stock'] ?>!</span><?php endif; ?>
                        </div>
...
<div id="productModal" class="fixed inset-0 bg-black/70 hidden flex items-center justify-center z-[999] p-3 backdrop-blur-sm transition-all">
    
    <div class="bg-white w-full max-w-5xl h-[85vh] md:h-auto md:max-h-[90vh] overflow-hidden relative shadow-2xl rounded-2xl md:rounded-3xl flex flex-col md:flex-row animate-[fadeIn_0.3s_ease-out]">
        ...
        <div class="w-full md:w-1/2 flex flex-col bg-white flex-1 overflow-hidden">
            ...
            <div class="p-6 md:p-10 overflow-y-auto custom-scrollbar flex flex-col flex-1">
                ...
                <div class="border-t border-gray-100 pt-6 mb-4">
                    ...
                    <div id="review-list" class="space-y-3 max-h-40 overflow-y-auto pr-2 custom-scrollbar mb-4"></div>
...
<script src="src/js/katalog.js?v=<?= time() ?>"></script> 
</body>
</html>