...
<main class="pt-28 lg:pt-36">
    ...
    <div class="mt-6 max-w-md relative">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-4 gap-y-8">
            ...
            <div class="group product-card transition-all flex flex-col h-full bg-white border border-gray-100 p-2">
                <div class="cursor-pointer flex-1" onclick="openModal('<?= htmlspecialchars($p['image']) ?>', '<?= htmlspecialchars(addslashes($p['title'])) ?>', '<?= number_format($p['price'], 0, ',', '.') ?>', '<?= htmlspecialchars(addslashes($p['description'])) ?>', <?= (int)$p['stock'] ?>, <?= $p['id'] ?>, <?= $p['price'] ?>)">
                    ...
                    <div class="space-y-1">
                        <h3 class="text-[12px] font-bold uppercase tracking-tight line-clamp-2"><?= htmlspecialchars($p['title']) ?></h3>
    ...
    <div class="bg-white w-full max-w-4xl overflow-hidden relative shadow-2xl rounded-sm flex flex-col md:flex-row max-h-[90vh] overflow-y-auto">
        ...
        <div class="w-full md:w-1/2 p-8 md:p-12">
            ...
            <div class="mt-8 border-t pt-6">
                ...
                <div id="review-list" class="space-y-4 max-h-40 overflow-y-auto mb-4 pr-2 custom-scrollbar">
                    </div>
            ...
            <div class="mt-8 flex gap-2">
                <button id="modalAddToCartBtn" class="flex-1 border border-gray-900 py-3 text-[10px] font-bold uppercase hover:bg-black hover:text-white transition">
                    ADD TO CART
                </button>
                <button onclick="window.location.href='checkout.php'" class="flex-1 bg-black text-white py-3 text-[10px] font-bold uppercase hover:bg-gray-800 transition">
                    PESAN SEKARANG
                </button>
            </div>
        </div>
